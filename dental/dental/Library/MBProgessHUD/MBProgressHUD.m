...
+ (instancetype)showHUDAddedTo:(UIView *)view animated:(BOOL)animated
          yPointViewBackground:(float)yPointView{
    
    MBProgressHUD *hud = [[self alloc] initWithView:view pointY:yPointView];
    hud.yPoint = yPointView;
    hud.removeFromSuperViewOnHide = YES;
    [view addSubview:hud];
    [hud showAnimated:animated];
    return hud;
...
- (id)initWithView:(UIView *)view
            pointY:(CGFloat)poitY{
    NSAssert(view, @"View must not be nil.");
    return [self initWithFrame:view.bounds pointY:poitY];
}
...
- (void)setupViews {
    ...
    if (self.yPoint > 0) {
        bound.origin.y = self.yPoint;
    }
    
    MBBackgroundView *backgroundView = [[MBBackgroundView alloc] initWithFrame:bound];
